...
const fermenterIcon = (props) => {
    ...
    <svg fill={fillColor[props.componentStatus]} id={props.id} xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" version="1.1" x="0px" y="0px" viewBox={props.view}  xmlSpace="preserve" width={props.width} height={props.height}>
        ...
        <text x="215" y ="100" writing-mode="lr" fill='white' text-anchor="middle" font-size="12px">{props.tank}</text>
        <text x="215" y ="120" writing-mode="lr" fill='white' text-anchor="middle" font-size="12px">{props.tank ? displayStatus(props.componentStatus) : null}</text>
        <text x="215" y ="140" writing-mode="lr" fill='white' text-anchor="middle" font-size="12px">{props.batch ? `Batch ${props.batch}` : null}</text>
      </svg>
    )
}
